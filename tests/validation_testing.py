...
def test_validate_missing_values():
    ...
    df2 = pd.DataFrame({"A": [1, 2, None, 4],"B": [4, None, 6, 7],"C": [7, 8, 9, 10]})
    ...
    expected2 = pd.DataFrame({"A": [1, 4],"B": [4, 7],"C": [7, 10]})
...
def test_validate_variance_in_dv():
    ...
    df = pd.DataFrame({"DV": [1, 2, 3, 4],"IV": ["A", "A", "B", "B"]})
    validate_variance_in_dv(df, dv="DV")
    # Case 2: No variation in DV
    df_no_var = pd.DataFrame({"DV": [5, 5, 5, 5],"IV": ["A", "A", "B", "B"]})
...
def test_validate_group_size():
    ...
    df = pd.DataFrame({"IV": ["A", "A", "B", "B", "B", "C", "C"],"Factor2": ["X", "X", "Y", "Y", "Y", "Z", "Z"],"DV": [1,2,3,4,5,6,7]})
    validate_group_size(df, iv="IV", factor2="Factor2", test="ANOVA")
    # Case 2: Group below minimal size (default test, n_min=2)
    df2 = pd.DataFrame({"IV": ["A", "A", "B"], "Factor2": ["X", "X", "Y"],"DV": [1,2,3]})
    ...
    df3 = pd.DataFrame({"IV": ["A"]*4 + ["B"]*5,"Factor2": ["X"]*4 + ["Y"]*5,"DV": list(range(9))})
    # A and X have 4 rows <5 and should raise error
    with pytest.raises(ValueError, match="Problematic cells"):
        validate_group_size(df3, iv="IV", factor2="Factor2", test="anova_tukey")
    # Case 4: anova_tukey with sufficient group size 
    df4 = pd.DataFrame({"IV": ["A"]*5 + ["B"]*5,"Factor2": ["X"]*5 + ["Y"]*5,"DV": list(range(10))})
    # Should pass without error
    validate_group_size(df4, iv="IV", factor2="Factor2", test="anova_tukey")

def test_validate_variable_type():
    df = pd.DataFrame({"cat1": ["A", "B", "A", "C"],"num1": ["1", "2", "3", "4"],
        "num2": [10, 20, 30, 40],"cat2": ["X", "Y", "X", "Y"]})
    # Case 1: Convert cat1 and cat2 to categorical, num1 to numeric
    result = validate_variable_type(df,categorical_list=["cat1", "cat2"],numeric_list=["num1"])
    ...
    df_invalid = pd.DataFrame({"cat": ["A", "B", "C"]}) 
    ...
    df_nonconvert = pd.DataFrame({"num": ["a", "b", "c"]})
    ...
    df_correct = pd.DataFrame({"cat": pd.Series(["X", "Y"], dtype="category"),"num": pd.Series([1.0, 2.0])})
