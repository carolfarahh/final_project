import pandas as pd
from src.eda import row_count_log, log_step, log_to_df
...
from src.eda import missingness_table, sample_for_plot
...
import pandas as pd
from src.eda import group_descriptives, sex_by_stage_table, age_by_stage_summary
...
import pandas as pd
import pytest
from src.eda import assert_filtered_gene_values


def test_assert_filtered_gene_values_passes_when_ok():
    df = pd.DataFrame({"Gene/Factor": ["mlh1", "msh3", "htt (somatic expansion)"]})
    assert_filtered_gene_values(df, "Gene/Factor", ["mlh1", "msh3", "htt (somatic expansion)"])
